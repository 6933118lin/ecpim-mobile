...
define(['jquery'], function($){
    var JQueryMobileSlide = function (arg_obj) {
        /*
         参数:
         el是jQuery选择器
        ...
        function slideAnimate(x, y) {
            if (Math.abs(x) >= y) {
                // $(".JQuerySlide").removeClass("JQuerySlideTransition");
                $(".JQuerySlide").css("-webkit-transform", "translate3d(0,0,0)");
            } else {
                // $(".JQuerySlide").addClass("JQuerySlideTransition");
                $(".JQuerySlide").css("-webkit-transform", "translate3d(" + x + "px,0,0)");
            }
        }
        ...
        function getSlideTimer(timeNum) {
            ...
            var slideTimer = setInterval(function() {
                var x = getTransform(".JQuerySlide").m41;
                x = x - ImgWidth;
        ...
        var transitionName = arg_obj["JQuerySlideTransition"] || "JQuerySlideTransition";
        var $JQuerySlideWrapper = arg_obj["JQuerySlideWrapper"] || $(".JQuerySlideWrapper");
        var $JQuerySlide = arg_obj["JQuerySlide"] || $(".JQuerySlide");
        var $JQuerySlide_img = arg_obj["JQuerySlide_img"] || $(".JQuerySlide_img");
        var timerNum = arg_obj["timerNum"] || 3000;

        $JQuerySlide_img.width($JQuerySlideWrapper.width());
        $JQuerySlide_img.height($JQuerySlideWrapper.height());

        var ImgWidth_arr_length = $JQuerySlide_img.length;
        var ImgWidth = $JQuerySlide_img.width();


        $JQuerySlide.width(ImgWidth * ImgWidth_arr_length);

        var JQuerySlideWidth = $JQuerySlide.width();
        // 轮播图终止位置
        var endPosition = JQuerySlideWidth - ImgWidth;
        ...
        touchDragMe($JQuerySlide, slideTimer);

        return slideTimer;
    }
    return JQueryMobileSlide;
});
